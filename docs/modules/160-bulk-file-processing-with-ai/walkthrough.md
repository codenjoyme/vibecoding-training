...
2. Copy this prompt and send it:
   ```
   Review all walkthrough.md files in ./docs/modules/ directory. For each file, check if it has:
...
2. Send this prompt to AI agent:
   ```
   For each walkthrough.md file in ./docs/modules/:
...
1. Navigate to module tools directory:
   ```powershell
   cd ./docs/modules/160-bulk-file-processing-with-ai/tools
   ```
...
2. Run the validation script:
   ```powershell
   python ./docs/modules/160-bulk-file-processing-with-ai/tools/validate_walkthroughs.py
   ```
...
3. Check results in any module:
   ```powershell
   cat ./docs/modules/010-installing-vscode-github-copilot/todo.md
   ```
