...
- **Inline code formatting for technical terms.** Wrap the following in backticks (`` ` ``):
  ...
  - `model` — wrap with backticks when referring to the AI language model concept (e.g. "the `model` generates", "the `model` does not compute"). Do not wrap in compound phrases like "mental model", in product names (`Claude Sonnet`, `GPT`), or in section headings.
  - `skill` — wrap with backticks when referring to the course-specific concept of an instruction+tool capability pair (Module 12 pattern). Do not wrap when used for generic human skills (e.g. "a valuable skill for any manager").
  - `tool` — wrap with backticks when referring to a script, program, or API call that an AI agent invokes for precise operations. Do not wrap in generic phrases like "useful tool" or "sharing across teams and tools".
