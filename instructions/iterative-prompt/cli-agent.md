...
2. Read these files in full so you know the rules:
   - [`.dark-factory/bricks/iterative-prompt/SKILL.md`](../../.dark-factory/bricks/iterative-prompt/SKILL.md) — the pattern (UPD/RESULT, format, atomic commits).
   - [`.dark-factory/bricks/iterative-prompt/runtime-cli.md`](../../.dark-factory/bricks/iterative-prompt/runtime-cli.md) — the CLI runtime rules (blocking watcher, no turn-end between UPDs).
3. Inspect the helm-log through the watcher before using grep, tail, or another ad hoc file-read method:
   ```
   python ./.dark-factory/bricks/iterative-prompt/scripts/watch_prompt.py {{HELM_LOG}}
   ```
...
2. Watcher exit code:
   - `0` → new UPD with `go` ready. Run `status.py --start --helm-log="<absolute-helm-log>" --upd-id="<UPD-id>"`, retain its `hash`, read the helm-log, find the last `## UPD[N]` without `### RESULT`, process it (implement the request, write `### RESULT (UPD[N])`, run `status.py --finish --started_from="<start-hash>"`, then make the atomic commit per [`SKILL.md`](../../.dark-factory/bricks/iterative-prompt/SKILL.md)).
...
- Pattern: [`.dark-factory/bricks/iterative-prompt/SKILL.md`](../../.dark-factory/bricks/iterative-prompt/SKILL.md)
- CLI runtime: [`.dark-factory/bricks/iterative-prompt/runtime-cli.md`](../../.dark-factory/bricks/iterative-prompt/runtime-cli.md)
- Watcher script: [`.dark-factory/bricks/iterative-prompt/scripts/watch_prompt.py`](../../.dark-factory/bricks/iterative-prompt/scripts/watch_prompt.py)
