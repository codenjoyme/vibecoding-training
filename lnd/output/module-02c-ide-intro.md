...
2. At the top of the chat panel, look for the mode selector. Make sure `Agent Mode` is selected (not `Ask Mode`). `Agent Mode` allows `Copilot` to take autonomous actions — creating files, running terminal commands, and making changes — not just answering questions.
