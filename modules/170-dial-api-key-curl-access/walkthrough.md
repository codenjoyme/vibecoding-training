...
In this walkthrough, you'll request your DIAL API key and make your first direct call to an AI model using cURL. This is the foundation for programmatic access to Company's AI infrastructure.
...
## Part 1: Understanding DIAL and API Keys
 ...
 DIAL is internal AI platform that provides access to various AI models (GPT-4, GPT-4o, Claude, etc.) through a unified interface. Instead of clicking in a chat UI, you can send requests programmatically using an API key.
...
**What is an API key?** Think of it as a password that identifies you when your scripts talk to DIAL servers. It's a long string like `1234567890abcdef1234567890abcdef` that you include in every request.

**Why do we need it?** To track usage, enforce security policies, and ensure compliance with companys's AI Policy.
...
1. Open your browser and navigate to DIAL:
   ```
...
3. Look for the footer text that mentions:
   ```
   Usage of DIAL must comply with the Prohibited Uses and Approval Steps in company's AI Policy. The information you share here is not disclosed to third-party companies. However, we anonymize and log all interactions for research purposes. >>> Request API key <<<. Report an issue. Version 0.40.5 Change log
   ```

4. Click on **"Request API key"** link

5. You'll be redirected to Support Portal with a pre-filled ticket template
...
9. You'll receive an email with your API key that looks like:
   ```
   Your DIAL API Key: 1234567890abcdef1234567890abcdef
...
DIAL is platform that provides **unified** access to multiple AI models through a simple REST API. It acts as a gateway to various AI providers including OpenAI, Anthropic, Google and others.
...
4. Create a simple cURL test script. Copy the script from module tools:
   ...
   **For Linux/macOS (Bash):**
   ...
   ```bash
   # View the script content
   cat modules/170-dial-api-key-curl-access/tools/test-dial-linux.sh
   ```
...
5. Copy the appropriate script to your test directory:
   ...
   **Linux/macOS:**
   ```bash
   cp modules/170-dial-api-key-curl-access/tools/test-dial-linux.sh work/dial-test/
...
- If still not visible, you may need VPN connection to company network
- Alternatively, search Service Portal for "DIAL API access"

### Issue: API key request ticket rejected or requires manager approval

**Solution:**
- Contact your direct manager to approve AI tool usage
- Reference Company AI Policy document for approval process
...
- Ensure you're on company network or connected to VPN
